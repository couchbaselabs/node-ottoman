...
function _decodeValue(context, type, data, name, schema) {
  if (data instanceof Object && data.$ref) {
    ...
    if (type instanceof CoreType) {
      ...
      for (var i = 0; i < data.length; ++i) {
        outArr[i] = _decodeValue(context, type.type, data[i], i, schema);
      }
      return outArr;
      ...
      var outObj = {};
      _decodeFields(context, type.fields, outObj, data, schema);
      return outObj;
    } else if (type instanceof ModelRef) {
      throw new Error(
          'Encountered a ModelRef field, but the data does not agree!');
    } else {
      console.log('Unknown field type.',i,type);
      throw new Error('Unknown field type.');
    }
  }
}

function _decodeFields(context, fields, obj, data, schema) {
  for (var i in data) {
    if (data.hasOwnProperty(i)) {
      ...
      if (!field) {
    		if( schema.filedsMustExist )
    			throw new Error('Could not find schema field for `' + i + '`.');
    		else
    		{
    		  field=Schema._makeFieldFromData(i,data[i]);
    		  fields.push(field);
    		  field = _findField(fields, i);
    		  if (!field) {
    				throw new Error('Could not determinate filed type from data for `' + i + '`.');
    		  }
    		  //console.log("creating field "+i,field)
    		}
      }

      obj[i] = _decodeValue(context, field.type, data[i],i, schema);
    }
  }
}

Schema.prototype.applyDataToObject = function(obj, data, schema) {
  _decodeFields(this.context, this.fields, obj, data, schema);
};

function _decodeUserValue(context, type, data, schema) {
  if (type instanceof CoreType) {
    if (type === dateCoreType) {
      return new Date(data);
    } else {
    ...
    var outArr = [];
    for (var i = 0; i < data.length; ++i) {
      outArr[i] = _decodeUserValue(context, type.type, data[i], schema);
    }
    return outArr;
  } else if (type instanceof FieldGroup) {
    if (!(data instanceof Object)) {
...
function _decodeUserFields(context, fields, obj, data, schema) {
  for (var i in data) {
    if (data.hasOwnProperty(i)) {
      var field = _findField(fields, i);
      if (!field) {
      ...
      obj[i] = _decodeUserValue(context, field.type, data[i], schema);
    }
  }
}

Schema.prototype.applyUserDataToObject = function(obj, data) {
  _decodeUserFields(this.context, this.fields, obj, data, this);
};
...
Schema._makeFieldFromData=function(){
      throw new Error('function Schema._makeFieldFromData should be replaced by ottoman._makeFieldFromData ');
}

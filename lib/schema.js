...
var debug = require('debug')('ottoman:Schema');
...
function CoreType(type) {
  debug('CoreType.constructor()');
  debug('  type: %s', type);
  this.type = type;
}
CoreType.prototype.inspect = function () {
  debug('CoreType.inspect()');
...
function FieldGroup() {
  debug('FieldGroup.constructor()');
  this.fields = [];
}

FieldGroup.prototype.create = function () {
  debug('FieldGroup.create()');
...
function SchemaIndexFn() {
  debug('SchemaIndexFn.constructor()');
  this.type = null;
  this.name = null;
  this.fields = null;
...
function RefDocIndexFn() {
  debug('RefDocIndexFn.constructor()');
...
Schema.prototype.namePath = function () {
  debug('Schema.namePath()');
  var path = this.context.nsPrefix() + this.name;
  debug('  path: %s', path);
  return path;
};
...
Schema.prototype.addDefIndexFn = function (name, indexDef) {
  debug('Schema.addDefIndexFn()');
  debug('  name: %s', name);
  debug('  indexDef: %O', indexDef);
...
Schema.prototype._tryAddDefQueryFn = function (name, queryDef) {
  debug('Schema._tryAddDefQueryFn()');
  debug('  name: %s', name);
  debug('  queryDef: %O', queryDef);
...
function _fieldTypeSearchNamedFields(fields, fieldName) {
  debug('_fieldTypeSearchNamedFields()');
  debug('  fields: %O', fields);
...
function _fieldTypeSearchWildcard(obj) {
  debug('_fieldTypeSearchWildcard()');
...
function _fieldTypeSearch(obj, pathObj, context) {
  debug('_fieldTypeSearch()');
  debug('  pathObj: %s', pathObj);
...
Schema.isCoreType = function (type) {
  debug('Schema.isCoreType()');
  debug('  type: %s', type);
  var result = type instanceof CoreType;
  debug('  %s', result);
  return result;
};

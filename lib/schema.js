...
Schema.prototype.addField = function(name, fieldDef) {
  ...
  if (fieldDef.type !== undefined) {
    if (typeof fieldDef.type === 'string') {
      var fieldType = this.context.typeByName(fieldDef.type);
      if (!fieldType) {
        throw new Error('Invalid type specified (' + fieldDef.type + ')');
      }

      if (fieldType instanceof CoreType || fieldDef.embed) {
        field.type = fieldType;
      } else {
        field.type = new ModelRef(fieldDef.type);
      }
    } else if (this.context.isModel(fieldDef.type)) {
      var fieldType = fieldDef.type.schema;
      if (fieldType instanceof CoreType || fieldDef.embed) {
        field.type = fieldType;
      } else {
        field.type = new ModelRef(fieldType.name);
      }
    } else {

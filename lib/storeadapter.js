...
CbStoreAdapter.prototype.ensureIndices = function(callback) {
  ...
  function ensureMrIndices() {
    var ddocs = [];
    for (var i in self.ddocs) {
      if (self.ddocs.hasOwnProperty(i)) {
        ddocs.push({
          name: i,
          data: self.ddocs[i]
        });
    ...
    var bucketMgr = self.bucket.manager();
  ...
  function ensureGsiIndices() {
    var fullQs = 'CREATE PRIMARY INDEX ON `' + self.bucket._name + '` USING GSI';
    self.bucket.query(couchbase.N1qlQuery.fromString(fullQs), function(err) {
      if (err) {
        callback(err);
        return;
      }

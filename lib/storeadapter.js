...
//callback(err)
CbStoreAdapter.prototype._createViewIndex = function(modelName, name, fields, callback) {
...
//callback(err)
CbStoreAdapter.prototype._createN1qlIndex = function(modelName, name, fields, callback) {
  this.gsis.push({
    modelName: modelName,
    indexName: name,
    fields: fields
...
CbStoreAdapter.prototype.ensureMrIndices = function(callback) {
  var ddocs = [];
...
CbStoreAdapter.prototype.ensureGsiIndices = function(callback) {
  var queries = [];
  ...
  for (var i = 0; i < this.gsis.length; ++i) {
    var gsi = this.gsis[i];
    queries.push(
        'CREATE INDEX ' + gsi.indexName + ' ' +
        'ON ' + this.bucket._name + '(' + gsi.fields.join(',') + ') ' +
...
CbStoreAdapter.prototype.ensureIndices = function(callback) {
  ...
  self.ensureMrIndices(function(err) {
    if (err) {
      callback(err);
      return;
    }

    self.ensureGsiIndices(function(err) {
      if (err) {
        callback(err);
        return;
      }

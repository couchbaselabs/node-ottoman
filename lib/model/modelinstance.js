...
ModelInstance.prototype.id = function() {
  ...
  // TODO: Maybe improve .query to support singluar lookup?
  return model.idField.query([this])[0];
};
...
ModelInstance.prototype.inspect = function(depth, options) {
  ...
  var idData = util.inspect(this.id());

  if (!this.loaded()) {
    return this.constructor.name + '(unloaded, id:' + idData + ')';
  }
...
ModelInstance.populate = function(instance, data) {
  ...
  // TODO: Validate populate matches the originating id.

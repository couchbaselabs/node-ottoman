 ...
 * @returns {string}
 */
ModelInstance.namePath = function (typeOnly) {
  return this.schema.namePath(typeOnly);
};
...
ModelInstance.prototype.id = function () {
  var $ = this.$;
  if (!$.loaded) {
    var keyPrefix = $.schema.namePath(false) + '|';
  ...
  var newKey = $.schema.namePath(false) + '|' + myId;
  if ($.key !== null) {
    if ($.key !== newKey) {
...
ModelInstance.prototype.toJSON = function () {
  ...
  var namespace = this.$.schema.context.namespace;
  var refs = {};
  ...
  _.deepMapValues(val, function (value, path) {
    ...
    if (path.match(/\$ref$/)) {
      ...
      if (modelType.indexOf(namespace + '::') > -1) {
        modelType = modelType.split(namespace + '::')[1];
      }
      refs[pathToRef.join('.')] = {
        $ref: modelType,
        $id: value
...
ModelInstance.ref = function (id) {
  return this.refByKey(this.schema.namePath(false) + '|' + id);
};

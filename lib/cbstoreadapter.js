...
CbStoreAdapter.prototype.get = function (key, callback) {
  ...
  /*
   * I'd like to use the asCallback() spread option but the reject case would
   * not be backwards compatible because it would do:
   *
   ...
   *
   * I had to do manual .then/.catch and callbacks for backwards compatibility.
   */
  return new Promise(function (resolve, reject) {
    ...
    self.bucket.get(key, function (err, res) {
      if (err) {
        return reject(err);
      }
...
CbStoreAdapter.prototype.store = function (key, data, cas, callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var saveHandler = function (err, res) {
      ...
      return resolve(res.cas);
    };

    if (cas) {
      self.bucket.replace(key, data, { cas: cas }, saveHandler);
    } else {
      self.bucket.insert(key, data, {}, saveHandler);
    }
  }).asCallback(callback);
};
...
CbStoreAdapter.prototype.remove = function (key, cas, callback) {
  ...
  var remove = Promise.promisify(this.bucket.remove, {context: this.bucket});

  if (cas) {
    return remove(key, { cas: cas }).asCallback(callback);
  } else {
    return remove(key).asCallback(callback);
  }
};
...
CbStoreAdapter.prototype.createIndex =
  function (type, modelName, name, fields, callback) {
    ...
    var createViewIndex = Promise.promisify(this._createViewIndex, {
      context: this
    });
    var createN1qlIndex = Promise.promisify(this._createN1qlIndex, {
      context: this
    });

    if (type === 'view') {
      return createViewIndex(modelName, name, fields).asCallback(callback);
    } else if (type === 'n1ql') {
      return createN1qlIndex(modelName, name, fields).asCallback(callback);
    } else {
      var err = new Error(
...
CbStoreAdapter.prototype.searchIndex =
  function (type, modelName, name, options, callback) {
    ...
    var searchViewIndex = Promise.promisify(this._searchViewIndex, {
      context: this
    });
    var searchN1qlIndex = Promise.promisify(this._searchN1qlIndex, {
      context: this
    });

    if (type === 'view') {
      return searchViewIndex(type, modelName, name, options)
        .asCallback(callback);
    } else if (type === 'n1ql') {
      return searchN1qlIndex(type, modelName, name, options)
        .asCallback(callback);
    } else {
      var err = new Error(
        'Couchbase does not support the index type specified `' +
...
CbStoreAdapter.prototype.count = function (type, modelName, options, callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var bucketName = self.bucket._name;
    var whereQs = '';
    if (expressions.length > 0) {
      whereQs = ' WHERE ' + expressions.join(' AND ');
    }

    var fullQs = 'SELECT COUNT(b) AS count FROM `' +
      bucketName + '` b' + whereQs;
    if (self.debug) {
      console.log('CbStoreAdapter::count~query', fullQs);
    }

    var query = self.couchbase.N1qlQuery.fromString(fullQs);
    self.bucket.query(query, function (err, res) {
      if (err) {
        return reject(err);
      }
  ...
  // For backwards compat, err/res cannot be undefined, must be null.
  // Otherwise, just use .asCallback to handle this...
  .then(function (res) {
    if (callback && callback instanceof Function) {
      callback(null, res);
    } else {
...
CbStoreAdapter.prototype.find = function (type, modelName, options, callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (options.limit !== undefined && options.skip !== undefined) {
      ...
      reject(new Error('Must have limit to use skip.'));
    }
    var sortQs = '';
    if (options.sort !== undefined) {
      ...
      if (Array.isArray(sortKeys)) {
        ...
        reject(new Error('Unknown sort value.'));
      }
    }
    ...
    if (options.consistency) {
      if (options.consistency === StoreAdapter.SearchConsistency.GLOBAL) {
        query.consistency(self.couchbase.N1qlQuery.Consistency.REQUEST_PLUS);
        ...
        query.consistency(self.couchbase.N1qlQuery.Consistency.NOT_BOUNDED);
      } else {
        callback(new Error('Unexpected consistency option.'), null);
        return;
      }

...
util.inherits(CbStoreAdapter, KvStoreAdapter);
...
CbStoreAdapter.prototype.dbGet = function (key, callback) {
  if (this.debug) {
    console.log('CbStoreAdapter::dbGet', key);
  }
...
CbStoreAdapter.prototype.dbStore = function (key, data, cas, callback) {
  if (this.debug) {
    console.log('CbStoreAdapter::dbStore', key, cas, data);
  }
...
CbStoreAdapter.prototype.dbRemove = function (key, cas, callback) {
  if (this.debug) {
    console.log('CbStoreAdapter::dbRemove', key, cas);
  }
...
function _buildViewMapFn(schema, fields) {
  ...
  for (var i = 0; i < fields.length; ++i) {
    var fieldType = schema.fieldType(fields[i]);
...
CbStoreAdapter.prototype._createViewIndex =
  function (model, name, fields, callback) {
    ...
    var mapFnStr = _buildViewMapFn(model.schema, fields);
...
CbStoreAdapter.prototype.createIndex =
  function (type, model, fields, callback) {
    ...
    if (type === 'refdoc') {
      // We handle refdocs differently than the rest of our index types since
      //  we don't need any data about them (and thus no ensureIndices needed).
      callback(null);
      return;
    } else if (type === 'view') {
      return this._createViewIndex(model, name, fields, callback);
    } else if (type === 'n1ql') {
      return this._createN1qlIndex(model, name, fields, callback);
    } else {
      callback(new Error(
...
CbStoreAdapter.prototype._searchViewIndex =
  function (type, model, name, options, callback) {
    ...
    // TODO: Handle ModelInstances being turned into $ref here...
    ...
    if (options.key) {
      ...
      for (var i = 0; i < options.key.length; ++i) {
        if (options.key[i] instanceof ModelInstance) {
          normalizedKey.push(options.key[i].id());
        } else {
          normalizedKey.push(options.key[i]);
        }
      }
    ...
    if (this.debug) {
      console.log('CbStoreAdapter::_searchViewIndex~info', query);
    }
    ...
    this.bucket.query(query, function (err, res) {
      ...
      for (var i = 0; i < res.length; ++i) {
        var keyInfo = self.serializer.decodeModelKey(res[i].id);
...
CbStoreAdapter.prototype.searchIndex =
  function (type, model, fields, options, callback) {
    ...
    if (type === 'view') {
      return this._searchViewIndex(type, model, name, options, callback);
    } else if (type === 'n1ql') {
      return this._searchN1qlIndex(type, model, name, options, callback);
    } else {
      callback(new Error(
        'Couchbase does not support the index type specified `' +
        type + '`.'));
 ...
 * @param {StoreAdapter~CountCallback} callback
 */
CbStoreAdapter.prototype.count = function (type, model, options, callback) {
  var modelName = model.schema.name;
 ...
 * @param {StoreAdapter~SearchCallback} callback
 */
CbStoreAdapter.prototype.find = function (type, model, options, callback) {
  var modelName = model.schema.name;
  ...
  if (options.filter) {
    ...
    _buildFilterExprs(filter, expressions);
  }
  var bucketName = this.bucket._name;
  var whereQs = '';
  if (expressions.length > 0) {

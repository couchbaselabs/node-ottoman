...
var debug = require('debug')('ottoman:CbStoreAdapter');
...
function _buildViewMapFn(schemaName, fields) {
  debug('_buildViewMapFn()');
  debug('  schemaName: %s', schemaName);
...
CbStoreAdapter.prototype._ensureMrIndices = function (callback) {
  ...
  function handler(err) {
    if (err) {
      proceed = ddocs.length;
      callback(err);
      return;
    }

    proceed++;
    if (proceed === ddocs.length) {
      callback(null);
      return;
    }
  }
...
CbStoreAdapter.prototype._ensureGsiIndices = function (callback) {
  ...
  var queryPromise = function (stringQuery) {
    debug('  stringQuery: %s', stringQuery);

...
function Ottoman(options) {
  ...
  this.namespace = '';
  this.store = null;
  this.serializer = new JsSerializer();
  this.models = {};
  this.types = {};
  this.delayedBind = {};
...
Ottoman.prototype._findModelsByDefIndex =
  function (type, model, fields, values, options, callback) {
    ...
    model.schema.store.searchIndex(
      type, model, fields, indexOpts, function (err, results) {
        if (err) {
          callback(err, null);
          return;
        }
...
Ottoman.prototype._countModels = function (model, filter, options, callback) {
  ...
  schema.store.count('n1ql', model, countOpts, function (err, res) {
    callback(err, res);
  });
};
...
Ottoman.prototype._findModels = function (model, filter, options, callback) {
  ...
  schema.store.find('n1ql', model, findOpts, function (err, res) {
    if (err) {
      callback(err, null);
      return;
    }
...
Ottoman.prototype._findModelsByQuery =
  function (mdlInst, model, query, callback) {
    ...
    this._findModelsByDefIndex(
      query.type, remoteType, [query.field], [mdlInst], options, callback);
  };
...
Ottoman.prototype._ensureModelIndices = function (model, callback) {
  ...
  for (var k = 0; k < modelIndices.length; ++k) {
    ...
    schema.store.createIndex(modelIndex.type, model,
      modelIndex.fields, handler);
  }
};
 ...
 * @param {Function} callback
 */
Ottoman.loadAll = function (items, callback) {
  ModelInstance.loadAll(items, callback);
};

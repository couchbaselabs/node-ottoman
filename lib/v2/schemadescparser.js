...
var ModelType = require('./types/modeltype');
var ModelRefType = require('./types/modelreftype');

/**
 * @interface SchemaTypeDb
 */

/**
 * @function
 * @name SchemaTypeDb#isSchemaRegistered
 * @param {Schema} schema
 * @returns boolean
 */
...
/**
 * @function
 * @name SchemaTypeDb#getTypeByName
 * @param {string} name
 * @returns {SchemaType}
 */
...
SchemaDescParser.prototype._parseTypeDesc = function(path, typeDesc) {
  // TODO: Parse the type description...
  ...
  if (typeof typeDesc === 'string') {
    ...
    return type;
  } else if (typeDesc instanceof Schema) {
    return new EmbeddedSchemaType(typeDesc);
  } else if (typeDesc instanceof Model) {
    return new ModelType(typeDesc);
  } else if (Array.isArray(typeDesc)) {
    if (typeDesc.length !== 1) {
      throw new Error('unexpected list descriptor format');
    }

    var itemType = this._parseTypeDesc(typeDesc[0]);
...
SchemaDescParser.prototype._parseFieldDesc = function(path, fieldDesc) {
  ...
  if (!(fieldDesc instanceof Object) ||

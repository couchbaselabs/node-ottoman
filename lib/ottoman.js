...
Ottoman.prototype._makeField = function (name, fieldDef) {
  ...
  if (fieldDef.hasOwnProperty('validator')) {
    switch (typeof fieldDef.validator) {
      case 'string':
        if (!this.validators[fieldDef.validator]) {
          throw new Error ('Validator "' + fieldDef.validator +
            '" for field "' + name + '" does not exist.');
        }
        field.validator = this.validators[fieldDef.validator]
        break;
      case 'function':
        field.validator = fieldDef.validator;
        break;
      case 'undefined':
        throw new Error('Undefined validator given for field "' + name + '".');
      default:
        throw new Error('Validators must be a function or a string matching ' +
          'the name of a registered custom validator function.');
    }
  } else {
    // Just in case anything relies on the property existing, backwards compat
 ...
 */
Ottoman.prototype.addValidators = function (validators) {
  ...
  for (var prop in validators) {
    if (typeof validators[prop] !== 'function') {
      throw new Error('Validator object properties must be functions.');
    }
    this.validators[prop] = validators[prop];
  }
};

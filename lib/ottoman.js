...
Ottoman.prototype.typeByName = function(type) {
  const coreType = Schema.coreTypeByName(type);
  if (coreType) {
    return coreType;
  }
...
Ottoman.prototype._parseFieldType = function(options) {
  ...
  if (Array.isArray(options)) {
    ...
    return new Schema.ListField(this._parseFieldType(options[0]));
  }
  ...
  if (!(options instanceof Object)) {
    throw new Error(
      'Unexpected field options type; expected either a ' +
        'valid ottoman type or an object with a type property'
    );
  }

  if (options.type === undefined && options.ref === undefined) {
    const grp = new Schema.FieldGroup();

    for (const i in options) {
      if (options.hasOwnProperty(i)) {
        const field = this._makeField(i, options[i]);
  ...
  if (options.ref) {
    ...
    if (typeof options.ref !== 'string') {
      ...
      const typeObj = this.typeByName(options.ref);
  ...
  if (typeof options.type === 'string') {
    const realType = this.typeByName(options.type);
    if (!realType) {
  ...
  if (this.isTypeDef(options.type)) {
    const typeDef = options.type;
    for (const j in typeDef) {
...
Ottoman.prototype._createSchema = function(name, schemaDef, options) {
  ...
  for (const i in schemaDef) {
    /* istanbul ignore else */
    if (schemaDef.hasOwnProperty(i)) {
      const field = this._makeField(i, schemaDef[i]);
      schema.addField(field);
    }
  }

  const indexFns = options.index;
  if (indexFns) {
    for (const j in indexFns) {
  ...
  const queryFns = options.queries;
  if (queryFns) {
    for (const k in queryFns) {
  ...
  if (options.id === undefined) {
    if (!schema.field('_id')) {
      const idField = this._makeField('_id', {
        type: 'string',
        auto: 'uuid',
        readonly: true
      });
 ...
 * @ignore
 */
Ottoman.prototype._findModelsByRefDocIndex = function(
  model,
  fields,
  values,
  options,
  ...
  const refKey = schema.refKey(fields, values);
 ...
 * @ignore
 */
Ottoman.prototype._findModelsByDefIndex = function(
  type,
  model,
  fields,
  values,
  ...
  const schema = model.schema;
  const mdlName = schema.namePath();
  const idxName = schema.indexName(fields);
  ...
  schema.store.searchIndex(type, mdlName, idxName, indexOpts, function(
    err,
    res
  ) {
    if (err) {
    ...
    for (let i = 0; i < res.length; ++i) {
      const obj = model.refByKey(res[i].id);
      results.push(obj);
    }
...
Ottoman.prototype._findModelByIndex = function(model, index) {
  ...
  let options = arguments[2 + index.fields.length + 0];
  let callback = arguments[2 + index.fields.length + 1];
...
Ottoman.prototype._countModels = function(model, filter, options, callback) {
  ...
  const countOpts = { filter: this._normFilter(filter) };
  for (const i in options) {
    if (options.hasOwnProperty(i)) {
...
Ottoman.prototype._findModels = function(model, filter, options, callback) {
  ...
  const findOpts = { filter: this._normFilter(filter) };
  for (const i in options) {
    if (options.hasOwnProperty(i)) {
 ...
 * @ignore
 */
Ottoman.prototype._findModelsByQuery = function(
  mdlInst,
  model,
  query,
  callback
) {
  const remoteTypeName = query.of;
  const remoteType = this.typeByName(remoteTypeName);
  if (!remoteType) {
    throw new Error(
...
Ottoman.prototype._buildModel = function(name, schemaDef, options) {
  ...
  const schema = this._createSchema(name, schemaDef, options);
  ...
  // jshint -W083
  const self = this;
  for (let i = 0; i < schema.indexFns.length; ++i) {
    const ifn = schema.indexFns[i];
    model[ifn.name] = this._findModelByIndex.bind(this, model, ifn);
  }
  for (let j = 0; j < schema.queryFns.length; ++j) {
    (function(qfn) {
...
Ottoman.prototype._buildAndRegisterModel = function(name, schemaDef, options) {
  ...
  // Build the model!
  const model = this._buildModel(name, schemaDef, options);
  this.models[name] = model;

  // Notify delayed binders!
  const delayedBinds = this.delayedBind[name];
  if (delayedBinds) {
    for (let i = 0; i < delayedBinds.length; ++i) {
      delayedBinds[i]();
    }
...
Ottoman.prototype._buildAndRegisterTypeDef = function(name, fieldDef) {
  ...
  const fieldData = this._makeField('', fieldDef);
...
Ottoman.prototype._ensureModelIndices = function(model, callback) {
  ...
  const modelIndices = [];
  for (let i = 0; i < schema.indices.length; ++i) {
    const index = schema.indices[i];

    if (!(index instanceof Schema.RefDocIndex)) {
      const idxName = schema.indexName(index.fields);

      const fields = [];
      for (let j = 0; j < index.fields.length; ++j) {
        const fieldName = index.fields[j];
        const fieldType = schema.fieldType(fieldName);
  ...
  for (let k = 0; k < modelIndices.length; ++k) {
    const modelIndex = modelIndices[k];
    schema.store.createIndex(
      modelIndex.type,
      schema.namePath(),
      modelIndex.name,
      modelIndex.fields,
...
Ottoman.prototype.getModel = function(name, prefixed) {
  if (prefixed) {
    ...
    const namePrefix = name.substr(0, prefix.length);
...
Ottoman.prototype.fromCoo = function(data, type) {
  ...
  const model = this.getModel(type, true);
  if (!model) {

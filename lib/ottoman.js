...
function modelQuery(query, options, callback) {
  ...
  var viewQuery = couchbase.ViewQuery.from(query.ddoc, query.view)
    .key(this.$key)
    .limit(query.limit);

  viewQuery.sort = query.sort;

  query.bucket.query(viewQuery, function(err, results) {
    if (err) {
      return callback(err);
    }
...
function getIndexInfo(index) {
  var ddocName = '__ottoidx_' + index.type;
  var viewName = 'by';
...
function verifyDdocView(bucket, ddoc, view, callback) {
  var timeoutErr = null;
  setTimeout(function() {
    timeoutErr = new Error('View creation timed out.');
  }, 8000);
  ...
  (function tryOnce() {
    ...
    var viewQuery = couchbase.ViewQuery.from(ddoc, view).limit(0);

    bucket.query(viewQuery, function(err, results) {
      if (err) {
        return tryOnce();
      }
      callback(null);
...
function setAndVerifyDdoc(bucket, ddoc, data, callback) {
  bucket.manager().upsertDesignDocument(ddoc, data, function(err) {
    if (err) {
      return callback(err);
    }
    ...
    for (var i in data.views) {
      remaining++;
      if (data.views.hasOwnProperty(i)) {
        verifyDdocView(bucket, ddoc, i, function(err) {
          if (err) {
            return callback(err);
          }
...
function registerDesignDocs(callback) {
  ...
  for (var i = 0; i < buckets.length; ++i) {
    ...
    for (var j = 0; j < views.length; ++j) {
      ...
      if (!ddocs[view.ddoc].views) {
        ddocs[view.ddoc].views = {};
      }

      ddocs[view.ddoc].views[view.name] = view.data;
    }

    for (var j in ddocs) {
      if (ddocs.hasOwnProperty(j)) {
        remaining++;
        setAndVerifyDdoc(buckets[i], j, ddocs[j], function(err) {
          remaining--;
          if (remaining === 0) {
            if (err) {
              return callback(err);

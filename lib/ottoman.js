...
/**
 * Make a field definition from Data to it's internal representation. - helps to load data of deleted fields.
 *
 * @param {String} name
 * @param {Object} options
 * @returns {*}
 ...
 * @ignore
 */
Ottoman.prototype._makeFieldFromData=function(name,options) {
	if(options instanceof Object)
	{
		if(options instanceof Array)
		{
			var field= new Schema.ListField(
				Ottoman.prototype._makeFieldFromData(undefined,options[0])
			);
		...
		else if(options._type)
		{
			...
			field.type = Schema.MixedType;
			field.validator = undefined;
			return field;
		}
		else
		{
			var field = new Schema.FieldGroup();

			for (var i in options) {
				if (options.hasOwnProperty(i)) {
					var subfield = Ottoman.prototype._makeFieldFromData(i, options[i]);
					field.fields.push(subfield);
				}
			}
			field.type = field;
			field.name = name; // sub object of array usually has no name
			return field;
		}
	}
	else
	{
		...
		{
			var type="Date";
			var realType = this.typeByName(type);
		...
		{
			var realType = this.typeByName(typeof options);
	...
	throw new Error('Could not deduce and create field type from data.');
};
...
Schema._makeFieldFromData=ottoman._makeFieldFromData.bind(ottoman);

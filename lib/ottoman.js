...
var debug = require('debug')('ottoman:Core');
...
Ottoman.prototype.nsPrefix = function () {
  debug('nsPrefix()');
  var ns = this.namespace ? this.namespace + '::' : '';
  debug('  namespace: %s', ns);
  return ns;
};
...
Ottoman.prototype.isTypeDef = function (type) {
  debug('isTypeDef()');
  var result = type instanceof TypeDef;
  debug('  %s', result);
  return result;
};
...
Ottoman.prototype.isModel = function (model) {
  debug('isModel()');
  var result = model.super_ === ModelInstance;
  debug('  %s', result);
  return result;
};
...
Ottoman.prototype._buildAndRegisterTypeDef = function (name, fieldDef) {
  debug('_buildAndRegisterTypeDef()');
  debug('  name: %s', name);
  debug('  fieldDef: %s', fieldDef);
  if (this.typeByName(name)) {
    throw new Error('A model with this name has already been registered.');
  }
...
Ottoman.prototype._ensureModelIndices = function (model, callback) {
  ...
  function handler(err) {
    if (err) {
      proceed = modelIndices.length;
      callback(err);
      return;
    }

    proceed++;
    if (proceed === modelIndices.length) {
      callback(null);
      return;
    }
  }

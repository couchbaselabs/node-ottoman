...
Ottoman.prototype._findModelByIndex = function (model, index) {
  ...
  if (arguments.length < 2 + index.fields.length) {
  ...
  if (options instanceof Function) {
    callback = options;
    options = {};
  } else if (arguments.length >= 2 + index.fields.length + 1) {
    callback = arguments[2 + index.fields.length + 1];
  }

  var self = this;

  return new Promise(function (resolve, reject) {
    function callback(err, mdlInsts) {
      if (err) {
        return reject(err);
      }
      return resolve(mdlInsts);
    }

    if (index instanceof Schema.RefDocIndexFn) {
      self._findModelsByRefDocIndex(model, fields, values, options, callback);
    } else {
      if (options.consistency === undefined) {
        options.consistency = index.consistency;
      }
      self._findModelsByDefIndex(
        index.type, model, fields, values, options, callback);
    }
  }).then(function (mdlInsts) {
    if (callback) {
      callback(null, mdlInsts);
    } else {
      return mdlInsts;
    }
  }).catch(function (err) {
    /*
      For backwards compatibility, we must manually call callback(err, null).
      If we relied on Promise.asCallback(), it would call callback(err) and
      the second argument to the callback would be undefined rather than null.
    */
    if (callback) {
      callback(err, null);
    } else {
...
Ottoman.prototype.validate = function (mdlInst, callback) {
  var validate = Promise.promisify(mdlInst.$.schema.validate, {
    context: mdlInst.$.schema
  });
  return validate(mdlInst).asCallback(callback);
};

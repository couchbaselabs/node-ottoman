...
Ottoman.prototype._findModelsByDefIndex = function(type, model, fields, values, callback) {
  ...
  schema.store.searchIndex(type, schema.name, idxName, {key:values}, function(err, res) {
    if (err) {
      callback(err, null);
      return;
    }
...
Ottoman.prototype._ensureModelIndices = function(model, callback) {
  ...
  for (var i = 0; i < modelIndices.length; ++i) {
    ...
    schema.store.createIndex(index.type, schema.name, index.name, index.fields, function(err) {
      if (err) {

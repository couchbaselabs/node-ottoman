...
Ottoman.prototype.resolveFieldDef = function(options) {
  ...
  if (options.type === undefined && options.ref === undefined) {
    ...
    for (var i in options) {
      if (options.hasOwnProperty(i)) {
        var field = this.resolveFieldDef(options[i]);
        field.name = i;
  ...
  if (options.ref) {
    ...
    options.type = new Schema.ModelRef(options.ref);
  }
  ...
  } else if (options.type instanceof Schema.FieldGroup) {
    // This is a field group, good to go.
  } else if (options.type instanceof Schema.ModelRef) {
    // This is a reference, good to go.
...
Ottoman.prototype._countModels = function(model, callback) {
  var schema = model.schema;
  var mdlName = schema.namePath();

  schema.store.countModels(mdlName, function(err, res) {
    callback(err, res);
  });
};
...
Ottoman.prototype._findModels = function(model, filter, options, callback) {
  ...
  schema.store.find('n1ql', mdlName, {filter:filter}, function(err, res) {
    if (err) {
      callback(err, null);
      return;
    }
...
Ottoman.prototype._buildModel = function(name, schemaDef, options) {
  ...
  model.find = this._findModels.bind(this, model);
  model.count = this._countModels.bind(this, model);

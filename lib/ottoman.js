...
function Ottoman(options) {
  ...
  if (options.models) {
    this.models = _.merge(this.models, options.models);
  }

  if (options.types) {
    this.types = _.merge(this.types, options.types);
  }
}
...
Ottoman.prototype.typeByName = function (type) {
  if (type.indexOf(this.namespace + '::') > -1) {
    type = type.split(this.namespace + '::')[1];
  }
...
Ottoman.prototype._createSchema = function (name, schemaDef, options) {
  ...
  if (options.namespace) {
    schema.namespace = options.namespace;
  }
...
Ottoman.prototype._findModels = function (model, filter, options, callback) {
  ...
  findOpts.namespace = schema.namespace;
...
Ottoman.prototype._ensureModelIndices = function (model, callback) {
  ...
  for (var i = 0; i < schema.indices.length; ++i) {
    ...
    if (!(index instanceof Schema.RefDocIndex)) {
      ...
      console.log('Push model index', { type: index.type, name: idxName, fields });
      modelIndices.push({
        type: index.type,
        name: idxName,
        fields: fields
  ...
  for (var k = 0; k < modelIndices.length; ++k) {
    ...
    schema.store.createIndex(modelIndex.type, schema.namePath(false),

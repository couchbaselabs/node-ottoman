...
describe('Model Indexes', function () {
  ...
  function _indexTestPromise(indexType, done) {
    ...
    ottoman.ensureIndices().then(function () {
      var x = new TestMdl();
      x.name = 'Frankie';
      x.company = 'Couchbase';
      var y = new TestMdl();
      ...
      return x.save()
        ...
        .then(function () {
          ...
          // empty [] and the test fails, which is a red herring.
          setTimeout(function () {
            TestMdl.findByName('Frankie', function (err, res) {
              assert.isNull(err);
              assert.isArray(res);
              assert.propertyVal(res, 'length', 1);
              var obj = res[0];
  ...
  it('should perform default string indexing successfully (promise)',
    function (done) {
      _indexTestPromise.call(this, undefined, done);
    }
  );

  it('should perform view string indexing successfully (promise)',
    function (done) {
      _indexTestPromise.call(this, 'view', done);
    }
  );

  it('should perform refdoc string indexing successfully (promise)',
    function (done) {
      _indexTestPromise.call(this, 'refdoc', done);
    }
  );

  it('should perform n1ql string indexing successfully (promise)',
    function (done) {
  ...
  if (ottoman.store instanceof ottoman.CbStoreAdapter) {
    it('should create index on _type when indexing for n1ql (promise)',
      function (done) {
        var couchbase = require('couchbase');
  ...
  it('should fail to have two identical refdoc keys (promise)',
    function (done) {
      var modelId = H.uniqueId('model');
  ...
  it('should allow two paths with the same name when undefined (promise)',
    function (done) {
      var modelId = H.uniqueId('model');
  ...
  it('should be ok to have two refdocs both undefined (promise)',
    function (done) {
      var modelId = H.uniqueId('model');
  ...
  it('should succeed with a previously changed refdoc key (promise)',
    function (done) {
      var modelId = H.uniqueId('model');
      var TestMdl = ottoman.model(modelId, {
        name: 'string',
  ...
  it('should fail for a missing refdoc value (promise)', function (done) {
    ...
    ottoman.ensureIndices()
      ...
      .then(function () {
        return TestMdl.findByName('Georgio');
      })
      .then(function (res) {
        assert.isArray(res);
        assert.propertyVal(res, 'length', 0);
  ...
  it('should fail to ensureIndex with an invalid index type (promise)',
    function (done) {
      var ottoX = new ottoman.Ottoman();
      ottoX.store = ottoman.store;
  ...
  it('should fail to search with an invalid index type (promise)',
    function (done) {
      var ottoX = new ottoman.Ottoman();
      ottoX.store = ottoman.store;

...
export const validateMinDate = (value: Date, min: Date | DateOption): string | void => {
  ...
  if (_minDate > value) {
    return (min as DateOption).message !== undefined
      ...
      : `Cannot allow dates before ${_minDate.toISOString()}`;
  }
};

export const validateMaxDate = (value: Date, max: Date | DateOption): string | void => {
  ...
  if (_maxDate < value) {
    return (max as DateOption).message !== undefined
      ...
      : `Cannot allow dates after ${_maxDate.toISOString()}`;
  }
};
